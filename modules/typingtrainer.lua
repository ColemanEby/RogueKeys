...
function TypingTrainer.new(text)
    ...
    self.typed = {}      -- Each element is a table: {char, correct = true/false}
...
-- Allow backspacing to remove the last character and update stats.
function TypingTrainer:backspace()
    ...
    if #self.typed > 0 then
        ...
        -- Recalculate current streak from the end of the typed array.
        local streak = 0
...
-- Mark the session as finished (only if the user has typed all characters).
...
-- Draws the header, the target text (with boxes around each letter), and stats.
